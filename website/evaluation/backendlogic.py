...
def select_two_teams():
    ...
    """
    list_of_all_teams = [x[1] for x in os.walk(models_url)]
    random_team_name = random.sample(list_of_all_teams[0], 2)
...
def add_new_result(winner_team, query, team_a, team_b, current_logged_in_user_id, team_a_result, team_b_result):
    ...
    team_a_score = get_or_create_score_by_team_name(team_a, result_id)
    team_b_score = get_or_create_score_by_team_name(team_b, result_id)

    add_team_outputs(result_id, team_a_result, team_b_result)
    update_scores(team_a_score, team_b_score, winner_team, result_id)


def get_or_create_score_by_team_name(team_name, result_id):
    ...
    Args:
        ...
        :param result_id: genereate new score against result
    ...
    try:
        ...
        score = Score(team_id=team_name, mu=trues_kill_rating.mu, sigma=trues_kill_rating.sigma, result_id=result_id)
        score.save()
    return score


def add_team_outputs(result_id, model_a_output, model_b_output):
    """ Saves the outputs generated by both teams against result

    :param result_id:
    :param model_a_output:
    :param model_b_output:
    :return:
    """
    team_output = TeamsOutput(result_id=result_id, model_a_output= model_a_output, model_b_output=model_b_output)
    team_output.save()
...
def update_scores(team_a_score, team_b_score, win_key, result_id):
    ...
    team_a_score.mu, team_a_score.sigma, team_a_score.result_id = new_rating_team_a.mu, new_rating_team_a.sigma, result_id
    team_b_score.mu, team_b_score.sigma, team_b_score.result_id = new_rating_team_b.mu, new_rating_team_b.sigma, result_id
